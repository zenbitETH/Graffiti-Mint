...
describe("🚩 Challenge 0: 🎟 Simple NFT Example 🤓", function () {
  ...
  describe("GraffitiMint", function () {
    if (process.env.CONTRACT_ADDRESS) {
      ...
      it("Should deploy GraffitiMint", async function () {
        const GraffitiMint = await ethers.getContractFactory(
          "GraffitiMint"
        );
        myContract = await GraffitiMint.deploy();
      });
    }
